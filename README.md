...
- 运营：创建项目、确认商品事实、分类上传并选择参考图、使用内容 AI、执行六类批量图片任务并逐图验收、派发与验收美工任务、执行生图七阶段、导出 JSON/Markdown/SKU CSV、操作本地 Mock ERP 草稿写回。
...
运营可以从左侧“批量图片工具”进入，也可以在商品项目详情点击“批量改图”。当前提供六个独立工具：

- 批量场景替换：同一商品参考图自然放入多张场景图。
- 批量印花提取：提取印花并校正透视，输出平面图候选。
- 批量改尺寸：智能重排到 1:1、3:4、4:3、3:2、2:3、16:9 或 9:16。
- 批量买家秀：将真实商品自然融入多张晒单场景。
- 批量角度裂变：按整体图和细节图数量生成统一镜头计划。
- 自定义批量：一条提示词应用到一批图，支持固定参考图和逐张配对参考图。

使用前必须确认商品信息卡；场景替换和买家秀还必须选择一张带文件哈希的“商品参考图”。一般任务一次最多处理 10 张，角度裂变最多 5 张输入。任务显示逐张进度和部分失败；成功候选必须逐项勾选真实性、结构与数量、Logo/文字、缩略图，并记录合规结论。检查记录不可覆盖，高风险或检查失败的候选不能确认。ZIP 只打包运营已确认的结果。删除采用软删除并保留审计链，排队中或运行中的任务不可删除。
